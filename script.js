...
form.addEventListener("submit", async function (event) {
    ...
    try {
        ...
        // Adicionar campos extras do FormSubmit
        formData.append('_subject', 'Nova solicitação - Instrutor Particular');
        formData.append('_template', 'table');
        formData.append('_captcha', 'false');

        // Enviar para o FormSubmit
        const resposta = await fetch('https://formsubmit.co/instrutorparticularcontato@gmail.com', {
            method: 'POST',
            body: formData
        });

        if (resposta.ok) {
            // Redirecionar para obrigado.html
        ...
        if (textoBotao) {
            textoBotao.textContent = "ENVIAR SOLICITAÇÃO";
        }
    }
});
